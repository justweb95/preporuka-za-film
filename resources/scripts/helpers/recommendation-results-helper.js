import { cyrillicFormat, durationFromat } from '@scripts/partials/textFormatingControler';

export async function poplateResult(resultObject) {
  let tmdbData = await resultObject

  let poster_holder = document.querySelector('#result-poster');
  let poster_holder_mobile = document.querySelector('#result-description-poster');
  let result_content_title = document.querySelector('#result-content-title');
  let result_description_text = document.querySelector('#result-description-text');
  let imdb_rating = document.querySelector('#imdb-rating');
  let result_genres_text = document.querySelector('#result-genres-text');
  let result_year_text = document.querySelector('#result-year-text');
  let result_cta_read_more = document.querySelector('.result-cta-read-more');
  let result_cta_trailer = document.querySelector('.result-cta-trailer');
  let movie_duration = document.querySelector('#result-duration-text');
  let youtube_player = document.querySelector('#youtube-player');

  let affiliate_amazon = document.querySelector('.affiliate-amazon');


  poster_holder.setAttribute('src', `https://media.themoviedb.org/t/p/w300_and_h450_bestv2/${tmdbData.poster_path}`)
  poster_holder_mobile.setAttribute('src', `https://media.themoviedb.org/t/p/w300_and_h450_bestv2/${tmdbData.poster_path}`)
  
  // result_content_title.textContent = tmdbData.original_title;
  result_content_title.textContent = cyrillicFormat(tmdbData.title);
  imdb_rating.textContent = parseFloat(tmdbData.vote_average.toFixed(1));

  result_description_text.textContent = cyrillicFormat(tmdbData.overview.substring(0, 300));

  result_genres_text.textContent = cyrillicFormat(tmdbData.genres);
  movie_duration.textContent = durationFromat(tmdbData.runtime);
  result_year_text.textContent = tmdbData.release_date.split('-')[0];


  // Set the href attribute to the result_cta_read_more element
  const fullUrl = tmdbData.url;
  result_cta_read_more.setAttribute('href', fullUrl);


  if (tmdbData.video_trailer && tmdbData.video_trailer.key) {
    youtube_player.setAttribute('src', `https://www.youtube.com/embed/${tmdbData.video_trailer.key}`);
  } else {
    result_cta_trailer.setAttribute('aria-disabled', 'true');
    result_cta_trailer.disabled = true;
  }

  let amazonProvider;

  if (tmdbData.movie_watch_on && Array.isArray(tmdbData.movie_watch_on.buy)) {
    amazonProvider = tmdbData.movie_watch_on.buy.find(provider => 
      provider.provider_name === 'Amazon Video' || provider.provider_name === 'Amazon Prime Video'
    );
  } else {
    // console.log("movie_watch_on or buy array does not exist.");
    
  }

  if (amazonProvider) {
    affiliate_amazon.setAttribute('href', `https://www.amazon.com/gp/video/primesignup?tag=preporukazafi-20`);
    affiliate_amazon.removeAttribute('aria-disabled');
    affiliate_amazon.style.opacity = '1';
    affiliate_amazon.style.pointerEvents = 'auto';
    affiliate_amazon.disabled = false;
  } else {
    affiliate_amazon.setAttribute('aria-disabled', 'true');
    affiliate_amazon.style.opacity = '.6';
    affiliate_amazon.style.pointerEvents = 'none';
    affiliate_amazon.disabled = true;
  }
  return true;
}

export function populateMultipleResults(resultsArray) {
  let container = document.querySelector('#results');
  if (!container) {
    container = document.createElement('article');
    container.id = 'results';
    container.className = 'question-holder container qa';
    document.body.appendChild(container);
  }
  container.innerHTML = '';

  // Dodaj H1
  const header = document.createElement('h1');
  header.className = 'questionnaire-header';
  header.textContent = 'Preporuka za tebe';
  container.appendChild(header);

  // Kreiraj Swiffy Slider container
  const sliderContainer = document.createElement('div');
  sliderContainer.className = 'swiffy-slider slider-item-show1 slider-nav-dark slider-nav-outside-expand';
  
  // Slider Control div 
  const sliderControle = document.createElement('div');
  sliderControle.classList = 'slider-controle';
  sliderControle.innerHTML = `
  <button type="button" class="slider-nav"></button>
  <button type="button" class="slider-nav slider-nav-next"></button>

  <ul class="slider-indicators">
      <li class="active"></li>
      <li></li>
      <li></li>
  </ul>
  `
  sliderContainer.appendChild(sliderControle);

  const sliderList = document.createElement('ul');
  sliderList.className = 'slider-container';
  sliderContainer.appendChild(sliderList);

  // Za svaki film kreiraj slider item
  resultsArray.slice(0, 3).forEach((tmdbData, index) => {
    const sliderItem = document.createElement('li');
    
    const dataHolder = document.createElement('div');
    dataHolder.className = 'result-data-holder';
    dataHolder.id = `slide${index+1}`;

    // Poster sekcija
    const posterWrapper = document.createElement('span');
    posterWrapper.className = 'result-poster-holder';
    const poster = document.createElement('img');
    poster.src = tmdbData.poster_path
      ? `https://media.themoviedb.org/t/p/w300_and_h450_bestv2/${tmdbData.poster_path}`
      : 'https://via.placeholder.com/300x450?text=No+Poster';
    poster.alt = tmdbData.title || 'No Poster';
    poster.className = 'result-poster';
    posterWrapper.appendChild(poster);

    // Content sekcija
    const contentWrapper = document.createElement('span');
    contentWrapper.className = 'result-content-holder';

    // Header sa naslovom i IMDb rating
    const contentHeader = document.createElement('div');
    contentHeader.className = 'result-content-header';
    
    const title = document.createElement('h2');
    title.className = 'result-content-title';
    title.textContent = tmdbData.title || 'Untitled';
    
    const ratingWrapper = document.createElement('span');
    ratingWrapper.className = 'result-imdb-rating-holder';
    
    // IMDb ikona SVG
    ratingWrapper.innerHTML = `
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M6.92439 0.707292C7.33541 -0.235765 8.66458 -0.235764 9.07559 0.707295L10.5194 4.02001C10.6856 4.40146 11.039 4.6667 11.4497 4.71823L14.9694 5.15988C15.947 5.28256 16.3492 6.48929 15.6429 7.1806L12.9628 9.8037C12.6802 10.0802 12.5543 10.481 12.6275 10.8708L13.3267 14.5951C13.5141 15.5931 12.4465 16.3487 11.5768 15.8335L8.59577 14.0675C8.22807 13.8497 7.77193 13.8497 7.40423 14.0675L4.4232 15.8335C3.55358 16.3487 2.48595 15.5931 2.67332 14.5951L3.37256 10.8708C3.44574 10.481 3.31982 10.0802 3.03725 9.8037L0.357138 7.1806C-0.349201 6.48929 0.0530059 5.28256 1.03065 5.15988L4.55031 4.71822C4.96095 4.6667 5.31436 4.40146 5.4806 4.02001L6.92439 0.707292Z" fill="#F6C700"/>
      </svg>
    `;
    
    const ratingText = document.createElement('p');
    ratingText.className = 'imdb-rating';
    ratingText.textContent = tmdbData.vote_average != null ? tmdbData.vote_average.toFixed(1) : 'N/A';
    
    const imdbIcon = document.createElement('img');
    imdbIcon.src = window.location.origin + '/preporuka-za-film/wp-content/themes/preporuka-za-film/public/images/questionnaire/imdb-icon.webp';
    imdbIcon.alt = 'Imdb Icon';
    
    ratingWrapper.appendChild(ratingText);
    ratingWrapper.appendChild(imdbIcon);
    
    contentHeader.appendChild(title);
    contentHeader.appendChild(ratingWrapper);

    // Info sekcija (žanrovi, trajanje, godina)
    const contentInfo = document.createElement('div');
    contentInfo.className = 'result-content-info';

    // Žanrovi
    const genresWrapper = document.createElement('span');
    genresWrapper.className = 'result-genres-holder';
    genresWrapper.innerHTML = `
      <svg class="result-genres-icon" width="18" height="19" viewBox="0 0 18 19" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M14.25 18.5H3.75C2.7558 18.4988 1.80267 18.1033 1.09966 17.4003C0.396661 16.6973 0.00119089 15.7442 0 14.75L0 4.25C0.00119089 3.2558 0.396661 2.30267 1.09966 1.59966C1.80267 0.89666 2.7558 0.50119 3.75 0.5H14.25C15.2442 0.50119 16.1973 0.89666 16.9003 1.59966C17.6033 2.30267 17.9988 3.2558 18 4.25V14.75C17.9988 15.7442 17.6033 16.6973 16.9003 17.4003C16.1973 18.1033 15.2442 18.4988 14.25 18.5ZM15 10.25H16.5V8.75H15V10.25ZM15 11.75V13.25H16.5V11.75H15ZM13.5 10.25H4.5V17H13.5V10.25ZM3 8.75H1.5V10.25H3V8.75ZM3 11.75H1.5V13.25H3V11.75ZM1.5 7.25H3V5.75H1.5V7.25ZM4.5 8.75H13.5V2H4.5V8.75ZM15 7.25H16.5V5.75H15V7.25ZM16.5 14.75H15V16.862C15.4372 16.7074 15.8159 16.4216 16.0844 16.0435C16.3529 15.6655 16.4981 15.2137 16.5 14.75ZM3 16.862V14.75H1.5C1.50192 15.2137 1.64706 15.6655 1.91557 16.0435C2.18407 16.4216 2.56282 16.7074 3 16.862ZM1.5 4.25H3V2.138C2.56282 2.29256 2.18407 2.57842 1.91557 2.95648C1.64706 3.33453 1.50192 3.7863 1.5 4.25ZM15 2.138V4.25H16.5C16.4981 3.7863 16.3529 3.33453 16.0844 2.95648C15.8159 2.57842 15.4372 2.29256 15 2.138Z" fill="#F57C36"/>
      </svg>
    `;
    
    const genresText = document.createElement('p');
    genresText.className = 'result-genres-text';
    genresText.textContent = tmdbData.genres && Array.isArray(tmdbData.genres)
      ? tmdbData.genres.map(g => g.name).join(', ')
      : (tmdbData.genres || 'N/A');
    
    genresWrapper.appendChild(genresText);
    contentInfo.appendChild(genresWrapper);

    // HR
    const hr1 = document.createElement('hr');
    contentInfo.appendChild(hr1);

    // Trajanje
    const durationWrapper = document.createElement('span');
    durationWrapper.className = 'result-duration-holder';
    durationWrapper.innerHTML = `
      <svg class="result-duration-icon" width="18" height="19" viewBox="0 0 18 19" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M9.00004 18.5001C7.22 18.5001 5.47994 17.9722 3.99989 16.9833C2.51984 15.9944 1.36628 14.5887 0.685091 12.9442C0.00389959 11.2997 -0.174331 9.49006 0.172937 7.74422C0.520206 5.99838 1.37738 4.39473 2.63605 3.13605C3.89473 1.87738 5.49838 1.02021 7.24422 0.672937C8.99006 0.325669 10.7997 0.5039 12.4442 1.18509C14.0887 1.86628 15.4944 3.01984 16.4833 4.49989C17.4722 5.97994 18.0001 7.72 18.0001 9.50004C17.9975 11.8862 17.0485 14.1739 15.3612 15.8612C13.6739 17.5485 11.3862 18.4975 9.00004 18.5001ZM9.00004 2.00001C7.51667 2.00001 6.06662 2.43988 4.83325 3.26399C3.59987 4.08811 2.63858 5.25945 2.07092 6.6299C1.50326 8.00035 1.35473 9.50836 1.64412 10.9632C1.93351 12.4181 2.64782 13.7545 3.69672 14.8034C4.74562 15.8523 6.08199 16.5666 7.53686 16.856C8.99172 17.1453 10.4997 16.9968 11.8702 16.4292C13.2406 15.8615 14.412 14.9002 15.2361 13.6668C16.0602 12.4335 16.5001 10.9834 16.5001 9.50004C16.4979 7.51158 15.707 5.60519 14.301 4.19913C12.8949 2.79307 10.9885 2.00219 9.00004 2.00001Z" fill="#F57C36"/>
        <path d="M5.95982 12.1976L5.16406 10.9255L8.24957 8.99279V4.99902H9.74958V9.82379L5.95982 12.1976Z" fill="#F57C36"/>
      </svg>
    `;
    
    const durationText = document.createElement('p');
    durationText.className = 'result-duration-text';
    
    if (tmdbData.runtime) {
      const hours = Math.floor(tmdbData.runtime / 60);
      const minutes = tmdbData.runtime % 60;
      durationText.textContent = hours > 0 ? `${hours}h ${minutes}min` : `${minutes}min`;
    } else {
      durationText.textContent = 'N/A';
    }
    
    durationWrapper.appendChild(durationText);
    contentInfo.appendChild(durationWrapper);

    // HR
    const hr2 = document.createElement('hr');
    contentInfo.appendChild(hr2);

    // Godina
    const yearWrapper = document.createElement('span');
    yearWrapper.className = 'result-year-holder';
    yearWrapper.innerHTML = `
      <svg class="result-year-icon" width="18" height="19" viewBox="0 0 18 19" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path fill-rule="evenodd" clip-rule="evenodd" d="M4.4935 3.50715V4.25685H5.99155V3.50715H11.9845V4.25685H13.4826V3.50715C15.1372 3.50719 16.4786 4.84858 16.4786 6.50325V13.9935C16.4786 15.6482 15.1372 16.9896 13.4825 16.9896H4.49416C2.83946 16.9896 1.49805 15.6482 1.49805 13.9935V8.00202L16.478 8.00202V6.50397L1.49805 6.50397V6.50325C1.49805 4.84877 2.8391 3.5075 4.4935 3.50715ZM5.99155 2.00909H11.9845V0.511719H13.4826V2.00909C15.9646 2.00914 17.9766 4.02122 17.9766 6.50325V13.9935C17.9766 16.4756 15.9645 18.4877 13.4825 18.4877H4.49416C2.0121 18.4877 0 16.4756 0 13.9935V6.50325C0 4.02142 2.01175 2.00945 4.4935 2.00909V0.511719H5.99155V2.00909Z" fill="#F57C36"/>
      </svg>
    `;
    
    const yearText = document.createElement('p');
    yearText.className = 'result-year-text';
    yearText.textContent = tmdbData.release_date ? tmdbData.release_date.split('-')[0] : 'N/A';
    
    yearWrapper.appendChild(yearText);
    contentInfo.appendChild(yearWrapper);

    // Opis filma
    const descriptionWrapper = document.createElement('div');
    descriptionWrapper.className = 'result-content-description';
    
    const descriptionText = document.createElement('p');
    descriptionText.className = 'result-content-description-text';
    descriptionText.textContent = tmdbData.overview
      ? tmdbData.overview.substring(0, 300) + (tmdbData.overview.length > 300 ? '...' : '')
      : 'Opis filma trenutno nije dostupan';
    
    descriptionWrapper.appendChild(descriptionText);

    // CTA dugmad
    const ctaWrapper = document.createElement('div');
    ctaWrapper.className = 'result-content-cta';
    
    const readMoreLink = document.createElement('a');
    readMoreLink.className = 'result-cta-read-more';
    readMoreLink.href = tmdbData.url || '#';
    readMoreLink.innerHTML = `
      <svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path fill-rule="evenodd" clip-rule="evenodd" d="M18 2.5H6C3.79086 2.5 2 4.29086 2 6.5V18.5C2 20.7091 3.79086 22.5 6 22.5H18C20.2091 22.5 22 20.7091 22 18.5V6.5C22 4.29086 20.2091 2.5 18 2.5ZM6 0.5C2.68629 0.5 0 3.18629 0 6.5V18.5C0 21.8137 2.68629 24.5 6 24.5H18C21.3137 24.5 24 21.8137 24 18.5V6.5C24 3.18629 21.3137 0.5 18 0.5H6ZM11 11.5V7.5H13V11.5H17V13.5H13V17.5H11V13.5H7V11.5H11Z" fill="#18BF7C"/>
      </svg>
      Saznaj više
    `;
    
    const trailerButton = document.createElement('button');
    trailerButton.className = 'result-cta-trailer';
    trailerButton.onclick = () => openTrailerPopUp(tmdbData.video_trailer);
    trailerButton.innerHTML = `
      <svg width="20" height="25" viewBox="0 0 20 25" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path fill-rule="evenodd" clip-rule="evenodd" d="M1.9241 4.33643V20.6636C1.9241 22.1931 3.63684 23.1054 4.91551 22.2569L17.2191 14.0934C18.3615 13.3354 18.3615 11.6646 17.2191 10.9066L4.91551 2.74306C3.63684 1.89465 1.9241 2.80693 1.9241 4.33643ZM0 4.33643V20.6636C0 23.7226 3.42546 25.5471 5.98281 23.8503L18.2864 15.6867C20.5712 14.1708 20.5712 10.8292 18.2864 9.31325L5.98281 1.14969C3.42546 -0.547141 0 1.27744 0 4.33643Z" fill="#18BF7C"/>
      </svg>
      Trejler
    `;
    
    ctaWrapper.appendChild(readMoreLink);
    ctaWrapper.appendChild(trailerButton);

    // Affiliate linkovi
    const affiliateWrapper = document.createElement('div');
    affiliateWrapper.className = 'result-content-affiliate';
    
    const amazonLink = document.createElement('a');
    amazonLink.href = "https://www.amazon.com/gp/video/primesignup?tag=preporukazafi-20";
    amazonLink.className = "affiliate-amazon";
    amazonLink.innerHTML = `
      <svg width="28" height="24" viewBox="0 0 28 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M21.2544 14.6446L20.4201 13.3693C20.1629 12.9763 20.0261 12.5182 20.0261 12.0501V6.6935C20.0261 4.55533 20.1828 0 14.0099 0C8.98833 0 7.43549 2.64661 6.95535 4.28356C6.76472 4.93344 7.21268 5.59516 7.89332 5.66462L9.83964 5.86321C10.2978 5.90993 10.7371 5.66946 10.9358 5.25856C11.3131 4.47813 11.9707 3.31586 14.0001 3.31586C14.7864 3.31586 15.8919 3.64471 15.8919 4.60623C15.8919 6.00137 15.89 6.66257 15.89 6.66257C15.8953 6.95372 15.6581 7.19262 15.3636 7.19262H13.4612C12.0258 7.19262 6.57129 7.9206 6.57129 13.3149C6.57129 18.7091 12.3592 18.0885 12.7435 17.9994C13.4605 17.8332 14.8038 17.348 16.0662 16.1575C16.2783 15.9575 16.6167 15.975 16.8009 16.2004L17.8204 17.448C18.2166 17.9329 18.9456 17.9839 19.407 17.559L21.0827 16.0155C21.4684 15.6601 21.5408 15.0823 21.2544 14.6446ZM12.887 14.344C10.9492 14.344 11.0044 12.6113 11.4516 11.7888C11.8699 11.0192 12.9752 9.65487 15.9426 9.56195L15.9686 10.9936C15.988 12.0578 15.4929 13.0801 14.6127 13.6925C14.0673 14.0721 13.4722 14.344 12.887 14.344Z" fill="white"/>
        <path d="M0.115567 19.2707C-0.179131 19.0074 0.141563 18.541 0.497823 18.7145C3.00166 19.9343 7.7154 21.8201 12.9259 21.8201C18.2505 21.8201 22.7051 20.5623 24.9471 19.7861C25.3116 19.6599 25.5743 20.1318 25.2742 20.3722C23.4954 21.797 19.6756 24.0011 13.0659 24.0011C6.48092 24.0011 2.20355 21.1367 0.115567 19.2707Z" fill="#FFAC35"/>
        <path d="M22.5996 18.5024C23.2942 17.9938 25.0199 17.117 27.599 17.8837C27.836 17.9541 27.9975 18.1709 27.9993 18.4156C28.0056 19.2524 27.8232 21.217 26.167 22.8497C26.052 22.963 25.8583 22.8517 25.9009 22.6967C26.1838 21.6666 26.7588 19.457 26.5763 19.1487C26.3672 18.7952 23.7533 18.7784 22.6943 18.7862C22.5397 18.7874 22.4754 18.5933 22.5996 18.5024Z" fill="#FFAC35"/>
      </svg>
      Gledaj na Amazonu
    `;
    
    const netflixLink = document.createElement('a');
    netflixLink.href = "#";
    netflixLink.className = "affiliate-netflix";
    netflixLink.innerHTML = `
      <svg class="preporuka-svg" width="28" height="24" viewBox="0 0 42 48" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path fill-rule="evenodd" clip-rule="evenodd" d="M21 42C32.598 42 42 32.598 42 21C42 9.40202 32.598 0 21 0C9.40202 0 0 9.40202 0 21V42L5.37209 47.3721V35.0277V21C5.37209 12.3689 12.3689 5.37209 21 5.37209C29.6311 5.37209 36.6279 12.3689 36.6279 21C36.6279 29.6311 29.6311 36.6279 21 36.6279C16.0595 36.6279 11.6545 34.3354 8.7907 30.7562V38.088C12.2312 40.5506 16.4463 42 21 42Z" fill="url(#paint0_linear_555_20)"/>
        <path fill-rule="evenodd" clip-rule="evenodd" d="M20.9993 33.2086C27.7424 33.2086 33.2086 27.7424 33.2086 20.9993C33.2086 14.2563 27.7424 8.79004 20.9993 8.79004C14.2563 8.79004 8.79004 14.2563 8.79004 20.9993C8.79004 27.7424 14.2563 33.2086 20.9993 33.2086ZM19.5471 13.07C20.2888 14.3548 19.8487 15.9975 18.5639 16.7392C17.2792 17.481 15.6365 17.0408 14.8947 15.7561C14.153 14.4714 14.5932 12.8286 15.8779 12.0869C17.1626 11.3451 18.8054 11.7853 19.5471 13.07ZM27.1169 26.1812C27.8586 27.4659 27.4184 29.1087 26.1337 29.8504C24.849 30.5922 23.2062 30.152 22.4645 28.8673C21.7228 27.5825 22.1629 25.9398 23.4477 25.198C24.7324 24.4563 26.3751 24.8965 27.1169 26.1812ZM23.4477 16.7392C22.163 15.9975 21.7228 14.3548 22.4645 13.07C23.2063 11.7853 24.849 11.3451 26.1338 12.0869C27.4185 12.8286 27.8586 14.4714 27.1169 15.7561C26.3752 17.0408 24.7324 17.481 23.4477 16.7392ZM14.8948 26.1812C14.153 27.4659 14.5932 29.1087 15.8779 29.8504C17.1627 30.5922 18.8054 30.152 19.5471 28.8673C20.2889 27.5825 19.8487 25.9398 18.564 25.198C17.2793 24.4563 15.6365 24.8965 14.8948 26.1812ZM25.8831 20.9997C25.8831 22.4831 27.0857 23.6857 28.5692 23.6857C30.0526 23.6857 31.2552 22.4831 31.2552 20.9997C31.2552 19.5162 30.0526 18.3136 28.5692 18.3136C27.0857 18.3136 25.8831 19.5162 25.8831 20.9997ZM13.4296 23.6857C11.9462 23.6857 10.7436 22.4831 10.7436 20.9997C10.7436 19.5162 11.9462 18.3136 13.4296 18.3136C14.9131 18.3136 16.1157 19.5162 16.1157 20.9997C16.1157 22.4831 14.9131 23.6857 13.4296 23.6857Z" fill="white"/>
        <defs>
        <linearGradient id="paint0_linear_555_20" x1="21" y1="0" x2="21" y2="47.3721" gradientUnits="userSpaceOnUse">
        <stop stop-color="#30C58A"/>
        <stop offset="1" stop-color="#4FC998"/>
        </linearGradient>
        </defs>
      </svg>
      Gledaj kod nas
    `;
    
    affiliateWrapper.appendChild(amazonLink);
    affiliateWrapper.appendChild(netflixLink);

    // Sastavi content wrapper
    contentWrapper.appendChild(contentHeader);
    contentWrapper.appendChild(contentInfo);
    contentWrapper.appendChild(descriptionWrapper);
    contentWrapper.appendChild(ctaWrapper);
    contentWrapper.appendChild(affiliateWrapper);

    // Sastavi data holder
    dataHolder.appendChild(posterWrapper);
    dataHolder.appendChild(contentWrapper);

    // Dodaj data holder u slider item
    sliderItem.appendChild(dataHolder);
    sliderList.appendChild(sliderItem);
  });

  container.appendChild(sliderContainer);

  // Dodaj dugmad za novu preporuku i donaciju
  const buttonsWrapper = document.createElement('div');
  buttonsWrapper.className = 'movie-btn';
  buttonsWrapper.innerHTML = `
    <button onclick="newAdvanceRecomendation()" class="new-recomendation" id="new-recomendation">
      <svg class="new-recomendation-icon" width="21" height="20" viewBox="0 0 21 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M17.9958 9.9831C18.0031 11.7298 17.404 13.425 16.3009 14.7793C15.1977 16.1336 13.6587 17.0631 11.9466 17.4092C10.2345 17.7554 8.45529 17.4967 6.91265 16.6773C5.37001 15.8579 4.1595 14.5285 3.48769 12.9162C2.81588 11.3038 2.7244 9.5082 3.22885 7.83587C3.7333 6.16355 4.80243 4.71808 6.2538 3.74614C7.70518 2.77419 9.44884 2.336 11.1873 2.50633C12.9257 2.67665 14.5512 3.44494 15.7863 4.6801L13.8222 6.64421H18.7546C18.8742 6.64432 18.9927 6.62084 19.1032 6.57511C19.2138 6.52938 19.3142 6.4623 19.3988 6.37772C19.4834 6.29313 19.5504 6.19269 19.5962 6.08216C19.6419 5.97162 19.6654 5.85315 19.6653 5.73352V0.801143L17.5542 2.91216C15.9146 1.28033 13.76 0.267413 11.4574 0.0459487C9.15474 -0.175516 6.84654 0.408174 4.92598 1.69759C3.00541 2.987 1.59128 4.90238 0.924469 7.11744C0.257659 9.33251 0.379419 11.7102 1.26901 13.8456C2.1586 15.981 3.76099 17.7419 5.80322 18.8284C7.84545 19.9148 10.2012 20.2597 12.4692 19.8042C14.7371 19.3487 16.777 18.121 18.2414 16.3302C19.7057 14.5394 20.5039 12.2963 20.5 9.9831H17.9958Z" fill="white"/>
      </svg>
      Daj mi drugu preporuku
    </button>
    <a href="{{ home_url() }}/anketa" class="start-over" id="start-over">
      <svg class="new-recomendation-icon" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <g id="Arrow / Arrow_Reload_02">
        <path id="Vector" d="M14 16H19V21M10 8H5V3M19.4176 9.0034C18.8569 7.61566 17.9181 6.41304 16.708 5.53223C15.4979 4.65141 14.0652 4.12752 12.5723 4.02051C11.0794 3.9135 9.58606 4.2274 8.2627 4.92661C6.93933 5.62582 5.83882 6.68254 5.08594 7.97612M4.58203 14.9971C5.14272 16.3848 6.08146 17.5874 7.29157 18.4682C8.50169 19.3491 9.93588 19.8723 11.4288 19.9793C12.9217 20.0863 14.4138 19.7725 15.7371 19.0732C17.0605 18.374 18.1603 17.3175 18.9131 16.0239" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        </g>
      </svg>
      Popuni upitnik ponovo
    </a>
  `;

  const donationWrapper = document.createElement('span');
  donationWrapper.className = 'result-donation';
  donationWrapper.innerHTML = `
    <p class="donation-text">Ako si zadovoljan onime što pruža ovaj sajt možete nas počastiti kafom:</p>
    <a href="https://www.paypal.com/ncp/payment/3B5JW7RMQFCWS" class="donation-link" id="donation-link">
      <svg class="donation-link-icon" aria-label="donation-link" xmlns="http://www.w3.org/2000/svg" width="62" height="21" viewBox="0 0 62 21" fill="none">
        <path fill-rule="evenodd" clip-rule="evenodd" d="M54.1545 3.75H52.4044V0.75H54.1545V3.75ZM50.6543 0.75H48.9042V3.75H50.6543V0.75ZM47.1542 0.75H45.4041V3.75H47.1542V0.75ZM41.0061 20.25H58.5069V18.75H41.001L41.0061 20.25ZM62.001 11.2501C61.9725 12.1578 61.5907 13.0186 60.9369 13.649C60.2831 14.2793 59.409 14.6296 58.5008 14.625H55.7453C55.1142 15.5604 54.4024 16.4388 53.6181 17.25H45.9002C43.4502 14.6689 41.7658 11.4578 41.035 7.975C40.9752 7.6173 40.9944 7.25085 41.0913 6.90136C41.1882 6.55188 41.3604 6.22785 41.5959 5.952C41.8462 5.65394 42.1588 5.41427 42.5116 5.24982C42.8644 5.08536 43.249 5.00009 43.6382 5H55.8757C56.2651 5.00036 56.6497 5.08593 57.0025 5.25069C57.3553 5.41545 57.6678 5.65542 57.918 5.95375C58.1534 6.22932 58.3256 6.55303 58.4227 6.90219C58.5197 7.25135 58.5392 7.61751 58.4798 7.975C58.4574 8.10331 58.4297 8.23163 58.4021 8.35994C58.392 8.40663 58.382 8.45331 58.3722 8.5H58.5008C60.6613 8.5 62.001 9.55437 62.001 11.2501ZM60.2509 11.2501C60.2509 10.5869 59.6637 10.25 58.5008 10.25H57.8997C57.598 11.1536 57.224 12.0315 56.7814 12.875H58.5008C58.9451 12.8797 59.3743 12.7138 59.6999 12.4115C60.0255 12.1092 60.2227 11.6935 60.2509 11.2501ZM20.7679 15.854C21.4079 16.238 22.1225 16.43 22.9119 16.43C23.8079 16.43 24.5972 16.1633 25.2799 15.63V16.318H27.4399V4.75L25.2479 5.118V8.526C24.9172 8.31267 24.5599 8.15267 24.1759 8.046C23.7919 7.92867 23.3919 7.87 22.9759 7.87C22.1652 7.87 21.4345 8.062 20.7839 8.446C20.1332 8.81933 19.6159 9.33133 19.2319 9.982C18.8585 10.622 18.6719 11.342 18.6719 12.142C18.6719 12.942 18.8585 13.6673 19.2319 14.318C19.6159 14.958 20.1279 15.47 20.7679 15.854ZM24.3679 14.366C24.0372 14.494 23.6692 14.558 23.2639 14.558C22.8052 14.558 22.3892 14.4513 22.0159 14.238C21.6532 14.0247 21.3652 13.7367 21.1519 13.374C20.9385 13.0113 20.8319 12.6007 20.8319 12.142C20.8319 11.6833 20.9385 11.2727 21.1519 10.91C21.3652 10.5367 21.6532 10.2487 22.0159 10.046C22.3892 9.83267 22.8052 9.726 23.2639 9.726C23.6585 9.726 24.0265 9.79533 24.3679 9.934C24.7092 10.0727 25.0025 10.2647 25.2479 10.51V13.758C25.0025 14.0247 24.7092 14.2273 24.3679 14.366ZM30.9576 15.9019C31.6403 16.2859 32.3976 16.4779 33.2296 16.4779C33.9016 16.4779 34.5043 16.3819 35.0376 16.1899C35.5816 15.9979 36.1096 15.6939 36.6216 15.2779L35.1816 13.9659C34.9576 14.1792 34.6856 14.3446 34.3656 14.4619C34.0563 14.5792 33.715 14.6379 33.3416 14.6379C32.9683 14.6379 32.6163 14.5632 32.2856 14.4139C31.9656 14.2646 31.6936 14.0566 31.4696 13.7899C31.2563 13.5232 31.1016 13.2246 31.0056 12.8939H37.1816V12.3179C37.1816 11.4646 37.0003 10.7019 36.6376 10.0299C36.2856 9.35789 35.8003 8.82456 35.1816 8.42989C34.563 8.03522 33.8536 7.83789 33.0536 7.83789C32.2536 7.83789 31.5283 8.02989 30.8776 8.41389C30.227 8.79789 29.7096 9.31522 29.3256 9.96589C28.9523 10.6166 28.7656 11.3472 28.7656 12.1579C28.7656 12.9686 28.963 13.6992 29.3576 14.3499C29.7523 15.0006 30.2856 15.5179 30.9576 15.9019ZM35.0216 11.3259H30.9736C31.059 10.9846 31.1923 10.6912 31.3736 10.4459C31.5656 10.1899 31.8003 9.99256 32.0776 9.85389C32.355 9.71522 32.6643 9.64589 33.0056 9.64589C33.3363 9.64589 33.635 9.72056 33.9016 9.86989C34.179 10.0086 34.4136 10.2059 34.6056 10.4619C34.8083 10.7072 34.947 10.9952 35.0216 11.3259ZM12.7125 16.3184L9.14453 7.98242H11.5285L13.8165 13.5344L16.0885 7.98242H18.4245L14.8405 16.3184H12.7125ZM2.192 15.9023C2.864 16.2863 3.616 16.4783 4.448 16.4783C5.29067 16.4783 6.048 16.2863 6.72 15.9023C7.392 15.5183 7.92 15.0009 8.304 14.3503C8.69867 13.6996 8.896 12.9689 8.896 12.1583C8.896 11.3476 8.69867 10.6169 8.304 9.96627C7.92 9.30493 7.38667 8.78227 6.704 8.39827C6.032 8.01427 5.28 7.82227 4.448 7.82227C3.616 7.82227 2.85867 8.01427 2.176 8.39827C1.504 8.78227 0.970667 9.30493 0.576 9.96627C0.192 10.6169 0 11.3476 0 12.1583C0 12.9689 0.197333 13.6996 0.592 14.3503C0.986667 15.0009 1.52 15.5183 2.192 15.9023ZM5.616 14.2543C5.27467 14.4676 4.88533 14.5743 4.448 14.5743C4.02133 14.5743 3.632 14.4676 3.28 14.2543C2.928 14.0409 2.65067 13.7529 2.448 13.3903C2.24533 13.0169 2.144 12.6063 2.144 12.1583C2.144 11.6996 2.24533 11.2889 2.448 10.9263C2.65067 10.5636 2.928 10.2756 3.28 10.0623C3.632 9.83827 4.02133 9.72626 4.448 9.72626C4.88533 9.72626 5.27467 9.83827 5.616 10.0623C5.968 10.2756 6.24533 10.5636 6.448 10.9263C6.66133 11.2889 6.768 11.6996 6.768 12.1583C6.768 12.6063 6.66133 13.0169 6.448 13.3903C6.24533 13.7529 5.968 14.0409 5.616 14.2543ZM37.001 18.75H0.000976562L0 20.25H37.001V18.75Z" fill="#F57C36"/>
      </svg>
    </a>
  `;

  container.appendChild(buttonsWrapper);
  container.appendChild(donationWrapper);

  // Inicijalizuj Swiffy Slider nakon što je DOM ažuriran
  setTimeout(() => {
    if (window.swiffyslider) {
      window.swiffyslider.init();
    }
  }, 100);

  return true;
}

// Pomoćna funkcija za otvaranje trejlera
function openTrailerPopUp(videoTrailer) {
  if (videoTrailer && videoTrailer.key) {
    window.open(`https://www.youtube.com/watch?v=${videoTrailer.key}`, '_blank');
  } else {
    alert('Trejler nije dostupan za ovaj film.');
  }
}
