<article 
  ...
  @if (!isset($single_movie_control) || $single_movie_control !== false)
    <div class="movie-card-menu-btn-holder">
      @if ($our_recommendations == "true")
      ...
      <button class="movie-card-btn-already-watched">
        <svg onclick="alreadywatched({{ $movie_ID }})" data-watched-id="{{ $movie_ID }}" class="watched-button" width="24" height="24" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <button class="movie-card-btn-favorites">
        <svg onclick="likeButtonHandler({{ $movie_ID }})" data-favorites-id="{{ $movie_ID }}" class="like-button" width="24" height="22" viewBox="0 0 24 22" fill="none" xmlns="http://www.w3.org/2000/svg">
